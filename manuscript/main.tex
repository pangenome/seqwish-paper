...
A scalable method to build pangenome graphs without such limitations would be a key functional step in pangenome construction and manipulation pipelines. \\
\textbf{Results:}
...
\FIXME{Conceptually, the pangenome graph relates genomes as walks through and underlying DNA sequence language encoded as a sequence graph.}
In a pangenome graph, variation can be understood in the context of any part of any included genome, which lets us avoid the problem of reference bias, or a limitation of analyses to only sequences that are very similar to a chosen reference genome \citep{Eizenga_2020}.

Surprisingly, existing methods to build pangenome graphs from large genome assemblies are reference-based.
Minigraph progressively builds a pangenome graph by layering an ordered series of input genomes onto a specific reference \citep{Li:2020}.
Cactus depends on a single guide-tree to structure pairwise alignments between genomes \citep{Armstrong:2020}.
Both methods limit computational costs by reducing the number of pairwise comparisons, but in turn their results depend on input genome order, selected reference, and guide-tree topology.
...
On this model, variation graphs add the concept of \textit{path} to embed linear sequences (\textit{e.g.}, genomes or haplotypes) into the graph \citep{Garrison_2018}.
...
\begin{proof}
By definition \ref{def:vg}, two paths have identical subsequences where they walk (or step) through the same series of oriented nodes (\textit{e.g.} $s_1 s_2 s_3$).
...
We do so non-redundantly by marking each used character in $Q$ in an auxiliary data structure (\textit{e.g.} a bitvector of $|Q|$).
...
In pratical, with large numbers of genomes, alignments dominate the computational costs.
