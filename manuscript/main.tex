...
Existing related approaches depend on a single reference, a specific ordering of genomes, or a \textit{de Bruijn} model, and may not describe the base-level mapping of genomes into the pangenome.
...
We demonstrate that our method scales to very large graph induction problems, and apply our work to build graphs from dozens of genomes from species in clades throughout the tree of life. \\
\textbf{Availability:}
...
$\mathcal{B}$ allows us to compute compacted node ids using efficient rank operation \citep{Gog_2014}.
...
To construct the graph we first generate alignments with \textit{wfmash} \citep{wfmash}, which is designed specifically for high performance all-to-all alignment of fully-assembled genomes.
...
This is useful when constructing pagnenome graphs, because, in contrast to methods that are based on $k$-mer chaining \citep{Li_2018}, it allows us to query the homology space of input genomes using two easily-interpretable parameters.
...
In table \ref{tab:experiments} we provide input and construced graph parameters for a single parameter setting of \textit{wfmash} and \textit{seqwish}, obtaining graph parameters with the ODGI toolkit \citep{Guarracino_2021}.
Figure \ref{fig:experiments} provides displays runtime versus graph size relative to the average input assembly length across the range of parameters chosen for each pangenome.
These provide a consistent set of insights.
Reducing the sensitivity of alignments by increasing the identity threshold results in a larger graph.
Filtering short matches results in a larger graph, and for higher divergence collections like \textit{H. pylori} tends to obliterate much of the homology information in the pangenome graph.
Reducing the size of $\mathcal{A}$ using these changes also greatly reduces runtime.
...
We find that our particular partitioning scheme---we compact the graph in chunks as described in \ref{sec:partitioning}, using 50 Mbp chunks in these experiments---does not allow us to complete the graph induction for $p=95$ and $k=0$ for the \textit{H. sapiens} set, nor for \textit{Z. mays} with $k < 49$, where we run out of working memory.
In practice, setting the chunk size lower tends to resolve this problem, but will also increase runtime.
To simplify comparisons between the different parameter settings, we have not re-run these settings with a different partition size.

\begin{table*}[!ht]
    \centering
    \caption{Performance of the graph induction algorithm.
        For each pangenome we report a single experiment with the given \textit{seqwish -k} filter setting of 49bp.
        }
...
Our experimental results domenstrate that we can apply our method to various collections of sequences and alignments.
...
In contrast to existing methods, which depend on particular structuring of their input \citep{Li:2020,Armstrong:2020}, \textit{seqwish} is unbiased in that it directly and uniformly represents sequence relationships given on input in the resulting graph.
Although we did not compare with \textit{de Bruijn} graph methods, which are also unbiased in their uniform treatment of input genomes \citep{Minkin_2016,Yu_2021}, we believe such methods are fundamentally different in that they collapse all exact matches of a given length $k$.
...
Our presentation is necessarily limited, so as to focus on and describe the unique problem of variation graph induction.
...
These topics lie outside of the scope of the presented work, wherein whe have focused on a key kernel which is a bottleneck in the pangenome graph construction process.
...
And this lossless property does not guarantee that the resulting graph is easy to work with or navigate---in practice find that downsteam processing is required to normalize the graph for many applications.
