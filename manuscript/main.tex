...
Paths $P = p_1\ldots p_{|P|}$ describe walks over node strands: $p_i = s_1 \ldots s_{|p_i|}$, storing a collection of genomes written terms of the sequence graph.

A variation graph models a set of pairwise alignments between its embedded paths.
The variation graph shows that two sequences match when they both ``step'' on the same node or series of nodes.
This is trivial given our definition.
If two sub-paths contain the same set of steps, they must have identical subsequences.
%Thus, to represent alignments in the graph, we built it by rendering matches into the graph
We can construct a variation graph from a set of pairwise alignments by guaranteeing that each matching pair of subsequences are represented in a single walk in the output graph.

We do so in a process based on three complementary graph models.
(\textit{1}) We transform the alignments and input sequences into an \textit{alignment graph} \citep{Kehr_2014}.
(\textit{2}) We then contract match components in this graph into single nodes of a \textit{sequence graph} \citep{Hein_1989}.
...
In doing so, we are able to derive edges and label the paths of a \textit{variation graph} \citep{Garrison_2018} which captures the pairwise alignments and all implied transitive relationships between them.
