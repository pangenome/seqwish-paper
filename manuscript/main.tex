...
%Finally, we derive edges and label the paths of the output variation graph by  

%A set of pairwise alignments imply a variation graph.

%We can see that pairs of aligned sequences would be represented as single 

%, however, to our knowledge, there is no existing method that can generate the variation graph or lossless string graph from large input alignments with arbitrary properties.

%We first formalize the graph induction algorithm that we will implement

%To explore this,  algorithm to do so that operates in external memory by conversion of the alignment set into an alignment graph and the subsequent use of this graph in the elaboration of the variation graph including paths representing the input sequences.
%The resulting graph is a lossless representation of the input and the alignments between them.
%To distinguish the approach from string graphs, which imply error correction, we call the result of this variation graph induction model the \emph{squish graph}.
