...
%We can save approximately $k$ match pairs per recorded range
...
It writes the graph in standard Graphical Fragment Assembly (GFA) format (\url{https://github.com/GFA-spec/GFA-spec}).
...
To reduce working memory requirements to an absolute minimum, we use a disk-backed version of the implicit interval tree that memory-maps the sorted array of intervals (\url{https://github.com/ekg/mmmulti}).
...
Although the application of the $k>$ filter can result in a graph that is relatively ``under-aligned'', we can further refine it through the application of local multiple sequence alignment \citep{Gao_2020}, or graph normalization (\url{https://github.com/marschall-lab/GFAffix}).
...
%    finally emitting the resulting graph in the standard Graphical Fragment Assembly (GFA) format (\url{https://github.com/GFA-spec/GFA-spec}).
...
To construct the graph we first generate alignments with \textit{wfmash} (\url{https://github.com/waveygang/wfmash}), a DNA sequence aligner designed specifically for high performance all-to-all alignment of fully-assembled genomes.
...
We compute 100 random permutations \citep{Williams2009-ou} of these assemblies and provide them to several graph construction methods, including \textit{minigraph} \citep{Li:2020}, \textit{TwoPaCo} \citep{Minkin_2016}, and \textit{seqwish} based on \textit{minimap2} \citep{Li_2018} or \textit{wfmash} (\url{https://github.com/waveygang/wfmash}) alignments.
