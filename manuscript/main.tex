...
However, by definition of $V$, it cannot contain \textit{less} match information than represented in $A$.
...
Due to our dependence on sorting, and the logarithmic-time cost of queries, crowth in $\mathcal{Q}$ drives $\Omega(\mathcal{Q} \log \mathcal{Q})$ growth in overall complexity.
As $N$ grows, the time complexity is dominated by the number of alignments, which in the case of our example is $O(N^2)$.
Clearly, for large numbers of highly-similar genomes, we may not require all pairs.
Various approaches could be used to reduce the size of $A$, but these lie outside the scope of this work.
...
As the analysis in section \ref{sec:bounds} shows, space requirements make it impractical to apply a trivial version of theorem \ref{thm:seqwish} to generate a large pangenome graph.
We seek a compression approach that will exploit redundancy in the input genomes to reduce the costs of the algorithm.
...
By encoding matches as pairs of ranges of characters, $r = (a, b) : a, b = (i, j) : i, j \in Q$, we can obtain a $\approx$ $k$-fold compression of $A$.
The same encoding can be used to replace the sequence-to-graph mapping $Z$.
Rather than pairs of characters in $Q$ and $\mathcal{N}$, we can record runs of matchhes between them as range matches.
...
%Taking $\mathcal{Q} = |Q|$, and assuming an all-to-all alignment of $N$ sequences in $A$ would give an approximate space bound of $O(2\mathcal{Q} + \mathcal{Q}N^2)$
%in the order of the length of sequences, the number of character matches in the alignments, and 
...
Using a compressed suffix array (CSA) \citep{Sadakane_2000} over sequence names, we build an index that maps between offsets in all genomes and offsets in $Q$ via lookup of the sequence name in the CSA.
