...
\address{
$^{\text{\sf 1}}$Department of Genetics, Genomics and Informatics, University of Tennessee Health Science Center, Memphis, 38163, Tennessee, USA \\
$^{\text{\sf 2}}$Genomics Research Centre, Human Technopole, Viale Rita Levi‑Montalcini 1, Milan, 20157, Italy
}
...
%$O[(|S|/N + 5|S|) \log |S| + |S|(2 + log(4))]$.
%$O(|S| \log |S| + |S|/N \log |S| + 2(|S| \log |S|) + 2(|S| \log 2|S| + |S|))$.
...
%in a ``graph vector'' $\mathcal{G} = v_1,v_2 \ldots v_{|\mathcal{V}|}$ containing the ordered characters of the graph.
%Any position in $\mathcal{G}$ so marked must be the start or end of a node.
We subsequently use these markings to subdivide $\mathcal{V}$ into a compacted version $\mathcal{V}'$ where compacted node boundaries are marked in a auxiliary bitvector $\mathcal{B} : |\mathcal{B}| = |\mathcal{V}|$ such that the first character in each compacted node is marked by a $1$ and other characters are marked $0$.
$\mathcal{B}$ allows us to compute compacted node ids using efficient rank operations \citep{Gog_2014}.
%such that $\forall_{i \in 1 \ldots |\mathcal{V}'|} |v_i| \ge 1$.
...
We have so far described our method for graph induction, provided detail on its implementation, and demonstrated its behavior over a set of pangenome graph building problems.
...
To focus on this questions, we examine the case of a single chromosome (chrV) of \textit{S. cerevisiae} for which we have 7 assemblies \citep{Yue_2017}.
...
In contrast, \textit{minigraph} develops a kind of partial order alignment over the minimizers (sparse $k$-mer set) of input genomes, and it progressively builds the graph by adding new variation from each genome in order.
We do expect it be order and reference (first input) biased, although the degree of bias requires experimentation to establish.

Finally, \textit{seqwish} itself is by definition unbiased with respect to input order, but if the alignments are affected by changes in order, the resulting graph will also change.

We use ODGI to compute the size of the resulting graphs \citep{Guarracino2022}.
When the graph length changes, it provides a clear indication that the graph itself has changed.
...
Conversely, if the graph length is the same for two large graphs, we assume that in most cases they really do have the same topology and sequence space, even if they may have different node identifiers and ordering in the serialization of the graph.
...
We further consider variance in the length, node count, and edge count of all graphs.
This shows that, as expected, \textit{minigraph}'s output depends markedly on input genome order.
We also find that \textit{minimap2} configuration we used is not order-independent, and can produce slightly different outputs when the reference and query sequences are ordered differently, demonstrating that \textit{seqwish} is only ``unbiased'' insofar as the input mappings are.
...
Although this particular study focuses on only a single small eukaryotic chromosome, it reveals that our basic understanding of reference bias in thes graph construction methods is correct.
...
%This leads to difficulty in building and designing pangenomes that is avoided by unbiased methods.

\input{fig_chrV_yeast.tex}

\begin{table}[!h]
    \centering
    \caption{Variance in graph properties across genome input order permutations.
    We report the mean ($\mu$) and variance ($\sigma^2$) in standard deviations ($\sigma$) for graph length (bp), node count (node), and edge count (edge).
