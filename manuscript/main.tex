...
Existing related approaches depend on a single reference, a specific ordering of genomes, or a \textit{de Bruijn} model, and may not describe the base-level alignment of genomes into the pangenome.
...
This is useful when constructing pangenome graphs, because, in contrast to methods that are based on $k$-mer chaining \citep{Li_2018}, it allows us to query the homology space of input genomes using two easily-interpretable parameters.
...
We find that our particular partitioning scheme (we compact the graph in chunks as described in~\ref{sec:partitioning}, using 50 Mbps chunks in all the experiments except for \textit{H. pylori} where we used chunks 10 Mbps long) does not allow us to complete the graph induction for $p=95$ and $k=0$ for the \textit{H. sapiens} set, nor for \textit{Z. mays} with $k \leq 29$, where we run out of working memory.
...
\begin{table*}[!ht]
    ...
    \caption{Performance of the graph induction algorithm.
        For each pangenome we report a single experiment with \textit{seqwish -k} filter set to 49bps.
        From left to right, the columns indicate the species, the number of sequences (that is, the number of contigs), number of haplotypes (that is, number of individuals), the sum of the length of all sequences in Gbps, the length of the short match filter applied in bps, the time in seconds and the amount of memory and disk space in Gbytes required for the graph induction, the length of the resulting graph in Gbps and the number its connected components.
        }
    \label{tab:experiments}
    \begin{tabular}{|l|l|l|l|l|l|l|l|l|l|}
        \hline
        species & sequences & haplotypes & fasta.Gbps & min.match.bps & time.seconds & memory.Gbytes & disk.Gbytes & graph.Gbps & components \\ \hline
