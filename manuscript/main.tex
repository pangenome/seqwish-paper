...
\subsection{Comparison to similar graph building methods}
...
Of existing methods, \textit{TwoPaCo} is the only that provides a variation graph in the same form as \textit{seqwish}.
...
We do not expect \textit{TwoPaCo} to be biased with respect to input genome order.
...
Finally, \textit{seqwish} itself is by definition unbiased with respect to input order, but if the alignments are biased due to changes in order, we should see effects on the resulting graph.

We use ODGI to compute the size of the resulting graphs \citep{Guarracino2022}, and use this as a proxy for graph identity.
